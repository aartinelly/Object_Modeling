...
public class UserService implements IUserService {
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){
        if (scoreOrder.equals(ScoreOrder.ASC)) {
            List<User> ascUser = userRepository.findAll();
             ascUser.sort(Comparator.comparing(User::getScore));
             return ascUser;
        }
        else if(scoreOrder.equals(ScoreOrder.DESC)){
            List<User> dscUser =  userRepository.findAll();
            dscUser.sort(Comparator.comparing(User::getScore).reversed());
            return dscUser;
        }
    ...
    public UserRegistrationDto withdrawContest(String contestId, String userName) throws ContestNotFoundException, UserNotFoundException, InvalidOperationException {
        ...
        UserRegistrationDto dto = new UserRegistrationDto(contest.getName(), user.getName(), RegisterationStatus.NOT_REGISTERED);
        
        if (user.checkIfContestExists(contest) && 
        (!contest.getContestStatus().equals(ContestStatus.IN_PROGRESS) || 
        !contest.getContestStatus().equals(ContestStatus.ENDED) ||
        !contest.getCreator().equals(user.getName()))) {
            user.deleteContest(contest);  
            userRepository.save(user); 
        } else {
            throw new InvalidOperationException("Contest not found for the user");
        }
